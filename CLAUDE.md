...
Group-level permissions (defined in `group_treasury.permissions.yml`):
- `view group_treasury`: View treasury tab (default: members)
- `propose group_treasury transactions`: Create transaction proposals (default: members)
- `sign group_treasury transactions`: Add signatures (default: admins)
- `execute group_treasury transactions`: Submit to blockchain (default: admins)
- `manage group_treasury`: Add/remove/reconnect treasury (default: admins)

Permission checks use Group's membership system via `$membership->hasPermission()`.
